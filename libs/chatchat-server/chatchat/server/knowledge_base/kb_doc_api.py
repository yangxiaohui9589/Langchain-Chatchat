...
def upload_docs(
        ...
        data_node_id: str = Form("", description="数据节点字符串"),
        override: bool = Form(False, description="覆盖已有文件"),
...
def update_docs(
        ...
        data_node_id: str = Body("", description="数据挂载节点id", examples=["数据挂载节点id"]),
    ...
    if not data_node_id:
        data_node_id = knowledge_base_name
    ...
    for file_name in file_names:
        ...
        if file_name not in docs:
            try:
                kb_files.append(
                    KnowledgeFile(
                        filename=file_name, knowledge_base_name=knowledge_base_name, security_level=security_level, data_node_id=data_node_id
                    )
                )
            except Exception as e:
    ...
    for status, result in files2docs_in_thread(
        ...
        if status:
            kb_name, file_name, security_level, data_node_id, new_docs = result
            kb_file = KnowledgeFile(
                filename=file_name, knowledge_base_name=knowledge_base_name, security_level=security_level, data_node_id=data_node_id
            )
